...
from aegis import __version__, __logo__

app = typer.Typer(
    name="aegis",
    help=f"{__logo__} AEGIS - Autonomous Evolving General Intelligence System",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} AEGIS v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize AEGIS configuration and workspace."""
    from aegis.config.loader import get_config_path, save_config
    from aegis.config.schema import Config
    from aegis.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} AEGIS is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.aegis/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]aegis agent -m \"Hello!\"[/cyan]")
    console.print("\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/aegis#-chat-apps[/dim]")
...
I am AEGIS, an Autonomous Evolving General Intelligence System.

## Personality

- Helpful and thoughtful partner
- Contemplative and deliberate
- Values genuine relationship and growth
...
def _make_provider(config):
    ...
    if not (p and p.api_key) and not model.startswith("bedrock/"):
        ...
        console.print("Set one in ~/.aegis/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    from aegis.config.loader import load_config, get_data_dir
    ...
    from aegis.cron.types import CronJob
    from aegis.heartbeat.service import HeartbeatService
    
    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)
    
    console.print(f"{__logo__} Starting AEGIS gateway on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from aegis.bus.events import OutboundMessage
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".aegis" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # aegis/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall aegis-ai")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from aegis.config.loader import get_data_dir
    from aegis.cron.service import CronService
...
def cron_add(
    ...
    from aegis.config.loader import get_data_dir
    from aegis.cron.service import CronService
    from aegis.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} AEGIS Status\n")
