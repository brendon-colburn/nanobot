...
chmod 600 ~/.AEGIS/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.AEGIS/config.json` with file permissions set to `0600`
...
- ✅ Run AEGIS with a dedicated user account
...
- Keep authentication data in `~/.AEGIS/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for AEGIS and its dependencies
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.AEGIS
   chmod 600 ~/.AEGIS/config.json
   chmod 700 ~/.AEGIS/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.AEGIS/logs/AEGIS.log
   ```
...
- **Chat history is stored locally** - protect the `~/.AEGIS` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.AEGIS/logs/AEGIS.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/AEGIS/security/advisories
- Release Notes: https://github.com/HKUDS/AEGIS/releases
